...
# sys.path.insert(1, '/home/mahya/PhD_project/UiO_ITU/OntologyGuidedPlanning/src/planGeneration/src')

# from run_program import my_module
###############################################################################
## DOMAIN CLASS
###############################################################################
class Domain(object):
...
class GroundedAction(object):
    ...
    # def __init__(self, action, *args, **kwargs):
        ...
        for prob_effects in action.probabilistic:
            ...
            **************************************************
            ...
            self.prob_effects.append(tuple([(utility_state, _effect_grounder(prob_effect[1])) for prob_effect in prob_effects]))

            # self.prob_effects.append(tuple([(prob_effect[0], _effect_grounder(prob_effect[1])) for prob_effect in prob_effects]))
            # print(self.prob_effects[0][0])
    ...
    def __str__(self, body=False):
        # Q: why planner do not check operator_str?
        if self == None:
            return None 
