(define (domain sonarAUV)
    ...
    (:predicates
        ...
        (ObjectAvoidanceFailure3 ?from - waypoint ?to - waypoint)
        (ObjectAvoidanceFailure4 ?from - waypoint ?to - waypoint)
        (ObjectAvoidanceFailure5 ?from - waypoint ?to - waypoint)
    ...
    (:action waypoint-following
        ...
        :effect (and 
            ...
            (probabilistic
                0.6 (when (ObjectAvoidanceFailure3 ?from ?to) (outOfRangeSonar1 ?sonar1))
            )
            (probabilistic
                0.7 (when (ObjectAvoidanceFailure4 ?from ?to) (outOfRangeSonar0 ?sonar0))
            )
            (probabilistic
                0.8 (when (ObjectAvoidanceFailure5 ?from ?to) (outOfRangeSonar0 ?sonar1))
            )
